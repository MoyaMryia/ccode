...
1. CLI 模式下能实际用
2. 有自动化测试
3. 现有测试套件全过（174 agent + 45 json + 32 http + 17 tui + 21 markdown + 5 tty + 8 e2e + 4 streaming；集成 37；`make mutate` 含 result/reasoning/webfetch 新 mutant 全 KILLED；test-tui-commands 与 test-tui-real 随 `ccode`/`ccode-tui` 暂停，`make ccode ccode-tui ccode-cli && make test-tui-real` 手动全绿）
