...
- 拒绝必须附上结构化工具结果，保证协议和后续模型轮次一致。命令过滤拒绝要写明命中了哪条规则；用户拒绝可附带输入的原因。

**暂时放宽**：允许 `--auto-approve` 和 `CCODE_AUTO_APPROVE=1` 跳过审批。

## 命令执行规则

- `bash` 是唯一的命令执行工具，走 `sh -c` 接受 shell 字符串；可选
  `timeout_ms`（默认 120000，上限 300000），校验前移到 prepare 层。
- 用 `fork()` + `execve()`，固定命令搜索策略。
- 用小号子进程环境，绝不继承 `CCODE_*` 凭证或父环境机密。
- 通过固定的工作区 fd 设 cwd，建独立进程组。
- 单调墙钟超时，终止整个进程组。
- 命令正常结束后也要清理其进程组里残留的后台进程（`kill(-pgid, SIGKILL)`），别让 `foo &` 把进程漏到工具调用之外；SIGTERM/SIGHUP 先清理命令进程组再按默认动作退出。
- 达到输出上限后继续排空 stdout/stderr，别让子进程写阻塞。
- 分别返回退出码、信号、超时、stdout/stderr、每流截断。
- git 不再有专用工具包装，一律经 `bash` 执行；所有命令子进程统一注入
  `GIT_CEILING_DIRECTORIES`（工作区父目录），防止 git 向上发现工作区外的
  仓库。
...
- 只读子代理自动放行其只读工具。只读子代理以 fork 并行、在各自的非前台进程组里共享父进程终端，一旦弹审批，子进程读控制终端会被 SIGTTIN 停住、父进程在 poll 上死等，且多个子进程争抢同一 stdin。只读子代理本就只能跑限工作区的只读工具（写工具未启用），无需逐次审批。读写子代理串行跑在父进程内，保留审批。
